...
function! airline#themes#{s:theme}#refresh()
    if &background == "dark"
        ...
        let Q1 = N1
        let W1 = [ '#1C1C1C', '#ef87b5', 234, 211 ]
        let E1 = [ '#1C1C1C', '#06bded', 234, 38 ]
        let R1 = [ '#1C1C1C', '#c3d773', 234, 149 ]
    ...
    let palette.q = airline#themes#generate_color_map(Q1, N2, N3)
    let palette.q.airline_error   = ER
    let palette.q.airline_warning = WI
    let palette.q.airline_term    = TE

    let palette.w = airline#themes#generate_color_map(W1, N2, N3)
    let palette.w.airline_error   = ER
    let palette.w.airline_warning = WI
    let palette.w.airline_term    = TE

    let palette.e = airline#themes#generate_color_map(E1, N2, N3)
    let palette.e.airline_error   = ER
    let palette.e.airline_warning = WI
    let palette.e.airline_term    = TE

    let palette.r = airline#themes#generate_color_map(R1, N2, N3)
    let palette.r.airline_error   = ER
    let palette.r.airline_warning = WI
    let palette.r.airline_term    = TE

    let palette.insert   = palette.e
    let palette.replace  = palette.r
    let palette.visual   = palette.w
